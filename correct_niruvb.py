# applies barycentric velocity + time corrections to NIR/UVB data
# separate from VIS data, which has its own process due to needing telluric corrections
# should probably try and unify these at some point

# intended to be run in a temporary folder containing the reduction subfolders for a set of NIR/UVB observations
...
def create_filename(input_filename):
    ...
    return('_'.join([object,date,time])+'_b.fits')
...
# scrape IDP_FLUX files from reduction folders
folders = [f for f in os.listdir() if os.path.isdir(f)]
folders.sort()

for i in range(len(folders)):
    folder = folders[i]
    file = [f for f in os.listdir(folder) if '_IDP_' in f][0]
    path = folder+'/'+file
    os.system(f'cp {path} .')
    ...
    print(f'BARY TIME CORR = {bary_time1}')
    ...
    print(f'CORRECTIONS APPLIED, SAVED TO {new_name}\n')
    hdul.close()
